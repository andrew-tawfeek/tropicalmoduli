...
class TestStability:
    ...
    def test_loop_degree(self):
        ...
        assert not g0.check_stability_at('0')
        assert not g1.check_stability_at('0')
        assert g2.check_stability_at('0')
    ...
    def test_markings(self):
        ...
        assert not g0.check_stability_at('0')
        assert not g2.check_stability_at('0')
        assert g3.check_stability_at('0')
    ...
    def test_large_one(self):
        ...
        for v in ['0', '1', '2']:
            assert g.check_stability_at(v)
        assert g.is_stable()

    def test_large_two(self):
        g = TropicalGraph({'a': ['a', 'b'], 'b': ['a', 'c'], 'c': ['b']}, {'c': 1}, {})
        assert g.check_stability_at('a')
        assert not g.check_stability_at('b')
        assert g.check_stability_at('c')
