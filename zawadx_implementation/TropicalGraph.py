...
class TropicalGraph:
    ...
    """
    def __init__(self, graph, weights: dict[str, int], markings: dict[str, int]) -> None:
        """
        ...
        Optionally, graph may also be an existing Multigraph, in which case the keys
        for weights and markings should again be vertices in graph.
        """
    ...
    def is_isom_to(self, other: Self) -> bool:
        ...
        """
        if (
            nx.faster_could_be_isomorphic(self.graph, other.graph) and 
            set(self.weights.values()) == set(other.weights.values()) and
            set(self.markings.values()) == set(other.markings.values())
           ):
